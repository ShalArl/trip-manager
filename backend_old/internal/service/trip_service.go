...
func (t *TripServiceImpl) CreateTrip(ctx context.Context, request *generated.CreateTripRequest, userID, userName, userEmail string) (*domain.Trip, error) {
	...
	// 2. Convert from generated type to shared
...
func (t *TripServiceImpl) UpdateTrip(ctx context.Context, request *generated.UpdateTripRequest, id, userID string) (*domain.Trip, error) {
	...
	// 4. Convert from generated type to shared (merges request with existing)
...
func (t *TripServiceImpl) ListRecentTrips(ctx context.Context, limit int, offset int) ([]*domain.Trip, int, error) {
	trips, total, err := t.tripRepository.ListRecentTrips(ctx, limit, offset)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to list recent trips: %w", err)
	}
